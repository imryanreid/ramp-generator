...
1. **Create the public GitHub repo** and push. The local repo exists with an
   initial commit on `main`; there's no remote yet.
2. **Create the Vercel project** and connect it to that repo. Framework preset:
   Vite. Build command `pnpm build`, output directory `dist` — both should be
